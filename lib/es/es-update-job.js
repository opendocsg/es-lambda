...
class ESUpdateJob {
  ...
  async getRepo(repoName, branch) {
    ...
    if (GITHUB_TOKEN) {
      ...
      auth.password = 'x-oauth-basic'
    }
    const { data: codeload } = await axios(codeloadURL, {
  ...
  async run(params) {
    ...
    const filesToIndex = markdownFiles.some(
      (x) => x.name.toLowerCase() === 'index.md'
    )
    ...
    for (const file of filesToIndex) {
      ...
      const sectionIndices = sections.map((section, i) => {
        const sectionId = {
          index: { _index: esIndex, _type: INDEX_TYPE, _id: `${entryId}_${i}` },
        }
        const sectionData = {
          doc_as_upsert: true,
          title: section.title,
